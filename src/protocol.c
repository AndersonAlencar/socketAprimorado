...
void show_history(struct Cell *cell){
    ...
    do{
        for(i = 0; i < cell->request.head[LENGTH]/SIZE_DOUBLE - 1 ; i++){
            if(cell->request.head[OPERATION] == OP_ADD) 
            	printf("%.2lf + ",cell->request.numbers[i]);
            
            else if(cell->request.head[OPERATION] == OP_SUBTRACT) 
            	printf("%.2lf - ",cell->request.numbers[i]);
            
            else if(cell->request.head[OPERATION] == OP_MULTIPLY) 
            	printf("%.2lf x ",cell->request.numbers[i]);
            
            else if(cell->request.head[OPERATION] == OP_DIVIDE) 
            	printf("%.2lf / ",cell->request.numbers[i]);
        }
...
struct Cell *allocate_new_cell(struct Cell *first_cell,struct Cell *last_cell){

        if(first_cell == NULL){
            first_cell = (struct Cell*)malloc(sizeof(struct Cell));
            first_cell->next = NULL;
            last_cell = first_cell;
        }
        else{
        	last_cell->next = (struct Cell*)malloc(sizeof(struct Cell));
